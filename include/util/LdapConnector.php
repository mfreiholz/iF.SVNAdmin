...
class LdapConnector {
   ...
   *          The entire DN of the user.
   * @param string $password
   *          The password to the specified user.
   ...
   * protocol to server.
   * This method have to be called on all DN and attribute
  ...
  public function objectRead($base_dn, $search_filter, $return_attributes) {
    ...
    $sr = ldap_read($this->connection, $base_dn, $search_filter, $return_attributes);
  ...
  public function objectSearch($base_dn, $search_filter, $attributes, $offset = 0, $num = -1, $ldapSizeLimit = 0) {
    ...
    $pageCookie = "";
    if ($num > 0) {
      $this->ldapSearchPageSize = $num;
    }
    do {
      if (function_exists("ldap_control_paged_result") && function_exists("ldap_control_paged_result_response")) {
        ldap_control_paged_result($this->connection, $this->ldapSearchPageSize, true, $pageCookie);
      }

      // Start search in LDAP directory.
      $sr = ldap_search($this->connection, $base_dn, $search_filter, $attributes, 0, $ldapSizeLimit);
      if (!$sr)
        break;
      ...
      if (function_exists("ldap_control_paged_result") && function_exists("ldap_control_paged_result_response")) {
        ldap_control_paged_result_response($this->connection, $sr, $pageCookie);
      }
  ...
  public function objectSearchResultCount($base_dn, $search_filter) {
    ...
    do {
      ...
      // Start search in LDAP directory.
      $sr = ldap_search($this->connection, $base_dn, $search_filter, array(), 0, 0);
      if (!$sr)
        break;

        // Get number of found results.
      $resultCount += ldap_count_entries($this->connection, $sr);
