...
class Htpasswd {
  ...
  public function error() {
    switch ($this->m_errno) {
      case 1:
        return "The user authentication file does not exist.";
      case 2:
        return "No READ permission on the user authentication file.";
      case 10:
   ...
   * specified salt.
   * If no salt is given then the function will use its own
   * generated salt.
   *
   * @param string $plainpasswd
   * @param string $salt
   ...
   * salt.
   * If no salt is given then the function will use it's own random
   * generated salt.
   *
   * @param string $plainpasswd
   * @param string $salt
  ...
  private function crypt_apr1_md5($plainpasswd, $salt = "") {
    ...
    $tmp = strtr(strrev(substr(base64_encode($tmp), 2)), "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", $translate_to);
