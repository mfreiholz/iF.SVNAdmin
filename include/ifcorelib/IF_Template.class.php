...
class IF_Template
{
  ...
  {
    // $strPattern can be the value：LocaleList, RepositoryParentList，ShowOptions，ShowDeleteButton，RepositoryList，UserList, AccessPathList
    if (!empty($strPattern)) // Using != NULL, because empty strings are valid
    {
      // give the array to $this->>m_replaements, then front page can display the data
      // save LocaleList, RepositoryParentList，ShowOptions，ShowDeleteButton，RepositoryList, UserList, AccessPathList to the list
