...
{
  public static $PERMISSION_NONE = '';  // No Permission
  public static $PERMISSION_READ = 'r'; // Read Permission
  public static $PERMISSION_READWRITE = 'rw'; // ReadWrite Permission

  private $SIGN_ALL_USERS = '*'; // all user
  private $GROUP_SIGN = '@'; // group sign
  private $GROUP_SECTION = 'groups'; // groups section
  private $ALIAS_SIGN = '&'; // alias sign
  private $ALIAS_SECTION = 'aliases'; // aliases section
  ...
  public function membersOfRepository($repository)
  {
    ...
    // find out '#section_desc' item
    $section_desc_index = array_search('#section_desc', $user_data);
    // remove the item that the value is '#section_desc'
  ...
  /**
   * Gets all groups list of which the user is a member.
   *
  ...
  /**
   * Gets all groups list of which the group is a member.
   *
  ...
  public function addRepositoryPath($repopath, $repodesc = null, $reason = null)
  {
    ...
    // write the repository or access path description into the config file
    // @see include/ifcorelib/IF_Config.class.php
    if (!empty($repodesc)) {
  ...
  /**
   * Removes the given subgroup from group.
   *
   * @param string $subgroupname
   * @param string $groupname
   *
