...
if (!$engine->isProviderActive(PROVIDER_REPOSITORY_EDIT)
	|| !$engine->getConfig()->getValueAsBoolean('GUI', 'RepositoryTreeEnabled', true)) {
	$engine->forwardError(ERROR_INVALID_MODULE);
}

$engine->checkUserAuthentication(true, ACL_MOD_REPO, ACL_ACTION_DOWNLOAD_TREE);
...
if ($varParentIdentifier == NULL || $varRepositoryName == NULL) {
	...
	try {
		...
		$engine->getRepositoryEditProvider()->tree($repositoryObject);
	}
	catch (Exception $e) {
